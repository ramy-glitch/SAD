...
<form id="criteria-names-weights-form">
    ...
    @for ($i = 1; $i <= $criteria; $i++)
        ...
        <div class="form-group">
            <label for="criteria_constraint_{{ $i }}">Enter constraint for criterion {{ $i }}:</label>
            <input type="number" id="criteria_constraint_min{{ $i }}" name="criteria_constraints[]" placeholder="min" required>
            <input type="number" id="criteria_constraint_max{{ $i }}" name="criteria_constraints[]" placeholder="max" required>
        </div>
